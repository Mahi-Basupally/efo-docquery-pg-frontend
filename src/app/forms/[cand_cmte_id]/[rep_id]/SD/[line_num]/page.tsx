...
export default function ScheduleDPage() {
  ...
  const [scheduleData, setScheduleData] = useState<ScheduleDResponse | null>(null);
  ...
  useEffect(() => {
    const fetchScheduleData = async () => {
      try {
        ...
        const response = await scheduleDApi.getScheduleDData(repId, lineNum, {
          page: currentPage,
          perPage: perPage,
        });
  ...
  const formatValue = (value: any, description: string): string => {
    ...
    if (description.toLowerCase().includes('balance') ||
        ...
        description.toLowerCase().includes('incurred') ||
  ...
  const breadcrumbItems = [
    ...
    { label: `Schedule D - Line ${lineNum}`, href: '' },
  ];
              ...
              <div className="row content__section">
                <div className="entity__figure row">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <button
                        ...
                        onClick={() => {
                          ...
                          a.href = url;
                          a.download = `schedule_D_${lineNum}_page_${currentPage}.json`;
                          a.click();
                  ...
                  <div className="heading--section heading--with-action">
                    <h3 className="entity__title">DEBTS AND OBLIGATIONS - Schedule D (Line #{lineNum})</h3>
                  </div>
                        ...
                        <tr>
                          {columns.map((col, index) => (
                            <th
                              key={index}
                              style={{
                                textAlign: col.description.toLowerCase().includes('balance') ||
                                          col.description.toLowerCase().includes('payment') ||
                                          col.description.toLowerCase().includes('incurred') ||
                        ...
                        {!data || data.length === 0 ? (
                          <tr>
                            <td colSpan={columns.length} style={{ textAlign: 'center', padding: '2rem' }}>
                              No transactions found for this line number.
                            </td>
                          </tr>
                        ) : (
                          data.map((row, rowIndex) => (
                            <tr key={rowIndex}>
                              {columns.map((col, colIndex) => {
                                // Use apiFieldName directly to access data
                                const value = row[col.apiFieldName];
                                const isNumeric =
                                  col.description.toLowerCase().includes('balance') ||
                                  col.description.toLowerCase().includes('payment') ||
                                  col.description.toLowerCase().includes('incurred') ||
                                  col.description.toLowerCase().includes('($)');
