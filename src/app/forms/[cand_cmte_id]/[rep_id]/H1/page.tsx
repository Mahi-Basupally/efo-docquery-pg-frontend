...
export default function H1Page() {
  ...
  // H1 shows one allocation-method record at a time (Version3Layout/
  // Version4Layout render a single `record`, not a table of rows) - page
  // is the only pagination control that matters here; perPage is fixed at
  // 1 rather than user-selectable.
  const H1_PER_PAGE = 1;
    ...
    const fetchScheduleData = async () => {
      try {
        ...
        const [response, reportResponse] = await Promise.all([
          h1Api.getH1Data(repId, { page: currentPage, perPage: H1_PER_PAGE }),
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ border: '1px solid #333', padding: '8px', textAlign: 'center' }}>
                    {formatPercent(record.houseSenatePercentFederalSupport)}
                  </td>
                  <td style={{ border: '1px solid #333', padding: '8px', textAlign: 'center' }}>
            ...
            <table className="data-table data-table--heading-borders data-table--entity u-margin--top dataTable no-footer">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style={{ border: '1px solid #333', padding: '8px', textAlign: 'center' }}>
                    {formatPercent(record.estimatedPercentFederalSupport)}
                  </td>
                  <td style={{ border: '1px solid #333', padding: '8px', textAlign: 'center' }}>
            ...
            <div className="slab slab--inline slab--neutral u-padding--left u-padding--right">
              <div className="row content__section">
                <div className="entity__figure row">
                  ...
                  {/* Pagination Controls - one H1 record per page */}
                  {pagination && (
                    <div
                      style={{
                        display: 'flex',
