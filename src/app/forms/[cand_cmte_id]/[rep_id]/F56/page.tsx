...
export default function F56Page() {
  ...
  const [scheduleData, setScheduleData] = useState<F56Response | null>(null);
  ...
  useEffect(() => {
    const fetchScheduleData = async () => {
      try {
        ...
        const response = await f56Api.getF56Data(repId, lineNum, {
          page: currentPage,
          perPage: perPage,
        });
        ...
        console.error('Error fetching F56 data:', err);
        setError(err instanceof Error ? err.message : 'Failed to load F56 data');
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [repId, lineNum, currentPage, perPage]);

  // Update document title when committee data is loaded
  useEffect(() => {
    const committeeName = scheduleData?.meta?.committeeDetails?.committeeName;
    const committeeIdFromData = scheduleData?.meta?.committeeDetails?.committeeId;

    if (committeeName) {
      document.title = `${committeeName} - EFO DocQuery`;
    } else if (committeeIdFromData) {
      document.title = `${committeeIdFromData} - EFO DocQuery`;
    } else {
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-red-800">{error || 'F56 data not found'}</p>
          </div>
        </div>
      </div>
    );
  ...
  // Build display name: use committeeName if available, otherwise construct from candidate name
  const displayName =
    committeeDetails.committeeName ||
    [
      committeeDetails.candidatePrefixName,
      committeeDetails.candidateFirstName,
      committeeDetails.candidateMiddleName,
      committeeDetails.candidateLastName,
      committeeDetails.candidateSuffixName,
    ]
      .filter(Boolean)
      .join(' ') ||
  ...
  const breadcrumbItems = [
    ...
    { label: `Form 5 - Line ${lineNum}`, href: '' },
  ];
              ...
              <div className="row content__section">
                <div className="entity__figure row">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <button
                        ...
                        onClick={() => {
                          ...
                          a.href = url;
                          a.download = `form5_${lineNum}_page_${currentPage}.json`;
                          a.click();
                  ...
                  <div className="heading--section heading--with-action">
                    <h3 className="entity__title">ITEMIZED CONTRIBUTIONS - Form 5 (Line #{lineNum})</h3>
                  </div>
                        ...
                        {!data || data.length === 0 ? (
                          <tr>
                            <td colSpan={columns.length} style={{ textAlign: 'center', padding: '2rem' }}>
                              No contributions found for this line number.
                            </td>
                          </tr>
                        ) : (
