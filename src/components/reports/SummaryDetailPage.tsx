...
export default function ReportDetailPage() {
    ...
    const fetchReportData = async () => {
      try {
        ...
        try {
          ...
          const apiEntityId = summaryData?.committee?.id || summaryData?.candidate?.candidateId;
          if (cmteId && apiEntityId && apiEntityId !== cmteId) {
            setCommitteeMismatch(true);
            setMismatchEntityId(apiEntityId);
            return;
          }
          ...
          console.warn('Error fetching summary:', apiClient.handleError(summaryError));
        }
      } catch (err) {
        console.warn('Error fetching report:', apiClient.handleError(err));
    ...
    return (
      <div style={{ padding: '2rem' }}>
        <div className="message message--error">       
          ...
          <p>Report FEC-{repId} does not belong to committee/candidate {cmteId}.</p>
  ...
  if (error || !report) {
    return (
      <div style={{ padding: '2rem' }}>
        <div className="message message--error">
          <h2 className="message__title">Report not found</h2>
          <p>Report ID: FEC-{repId}</p>
          <div className="message--alert__bottom">
            <ul className="list--buttons">
              <li>
                <Link className="button--standard" href={cmteId ? `/forms/${cmteId}` : '/forms'}>
                  Back to committee
                </Link>
              </li>
            </ul>
