...
export interface ReportTableColumn { key: string; label: string; align?: 'left' | 'right' | 'center'; width?: string; }
export interface ReportTableRow {
  id: string;
  cells?: Record<string, ReactNode>;
  fullWidth?: { lead?: ReactNode; leadColSpan?: number; content: ReactNode; contentColSpan?: number };
  className?: string;
...
export default function ReportTable({ id, title, subtitle, columns, rows, collapsible = true, expanded, onToggleExpanded, defaultExpanded = true, emptyMessage = 'No data available.' }: ReportTableProps) {
  ...
  const toggle = () => { onToggleExpanded?.(); if (!isControlled) setInternalExpanded(prev => !prev); };
  const alignOf = (align?: 'left' | 'right' | 'center'): 'left' | 'right' | 'center' => align || 'left';
    ...
    <div style={{ overflowX: 'auto', maxWidth: '100%', display: 'block' }}>
      <table id={id} style={{ width: '100%', fontSize: '16px' }} className="data-table data-table--heading-borders data-table--entity u-margin--top dataTable no-footer">
        ...
        <tbody style={{ display: isExpanded ? '' : 'none' }}>
          {rows.length === 0 ? <tr><td colSpan={columns.length} style={{ padding: cellPadding, fontSize: cellFontSize, textAlign: 'center', color: '#6b7280' }}>{emptyMessage}</td></tr> : rows.map(row => (
            <tr key={row.id} id={row.id} className={row.className} style={row.style}>
              {row.fullWidth ? <>
                {row.fullWidth.lead !== undefined && <td colSpan={row.fullWidth.leadColSpan ?? 1} style={{ textAlign: 'left', fontWeight: 'bold', padding: cellPadding, fontSize: cellFontSize }}>{row.fullWidth.lead}</td>}
                <td colSpan={row.fullWidth.contentColSpan ?? columns.length - (row.fullWidth.lead !== undefined ? 1 : 0)} style={{ textAlign: 'left', fontWeight: 'bold', padding: cellPadding, fontSize: cellFontSize }}>{row.fullWidth.content}</td>
              </> : columns.map(col => <td key={col.key} style={{ textAlign: alignOf(col.align), padding: cellPadding, fontSize: cellFontSize }}>{row.cells?.[col.key]}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
